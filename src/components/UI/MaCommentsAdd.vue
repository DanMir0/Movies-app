...
const {user} = useUser()
const {addComment} = useComments(movieId)

const highlightStars = (stars) => {
    highlightedStars.value = stars;
};
...
const changeRating = (grade) => {
    rating.value = grade
    highlightedStars.value = grade
}

const onClickAddComment = async () => {
    await addComment(user.value.uid, user.value.displayName, inputComment.value, rating.value)
    inputComment.value = ''
    rating.value = 0
}
</script>
...
<template>
    <div class="comments__add">
        <div class="rating">
            <div class="star" v-for="i in 10" :key="i" @click="changeRating(i)">
                <svg
